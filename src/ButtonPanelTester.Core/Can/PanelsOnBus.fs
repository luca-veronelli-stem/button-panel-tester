...
/// `specs/003-panel-discovery/data-model.md` §4.1. Keying
/// by `PanelUuid` is what guarantees coalescing (FR-002): re-broadcasts
...
module PanelsOnBus =
    ...
    /// `PanelDiscoveryService` and as the post-`clear` value for FR-008
    /// link-loss handling.
    ...
    /// the receive timestamp `now`. Per `data-model.md` §4.2:
    ...
    /// `PanelDiscoveryService` on the Connected → Disconnected transition
    /// (FR-008) so the GUI's Panels-on-bus list does not display
