...
- Re-key button-press observability to the **button-state heartbeat**, not WHO_I_AM discovery — fixes the bench-surfaced defect that a baptized panel, silent on WHO_I_AM (`AAS_STAND_BY`), was invisible to the test. The observer reassembles per source CAN ID and emits the panel's variant alongside each accepted frame (the #270 accept rule keyed acceptance + variant off the heartbeat's arbitration ID; superseded by the senderId rule of #296 below); the service/GUI key observability, variant, and panel-loss off button-state-frame recency (bench-tunable thresholds), auto-targeting the single heartbeating panel and dropping the `IPanelDiscoveryService` dependency from the button-press path ([#270](https://github.com/luca-veronelli-stem/button-panel-tester/issues/270)).
...
- Accept the button-state heartbeat by its **payload senderId**, not its CAN arbitration ID. The heartbeat's arbitration ID is the **destination** — the address of the master that baptized the panel (`app.srid = MotherBoardAddress`) — so a panel baptized by THIS tool heartbeats on the tool's own SRID `0x00000008`, the very id the #270 rule dropped: the first tool-baptized bench panel was invisible and the GUI never enabled. The panel's own address rides in the reassembled packet's senderId (bytes 1–4, big-endian) and its machineType byte (bits 23–16) is the variant. The observer now accepts a completed packet iff command `0x0002` + a recognised button-state address + a senderId decoding to a known `Marketing` variant, with no arbitration-ID pre-filter (WHO_I_AM drops on its command, the virgin sentinel on its address). Lean-mechanised (`variant_from_sender_id`, `arbitration_id_irrelevant`; no `sorry`) and FsCheck-mirrored; the June machine-baptized captures had worked by coincidence — a machine master shares the machineType byte with its keyboard ([#296](https://github.com/luca-veronelli-stem/button-panel-tester/issues/296)).
