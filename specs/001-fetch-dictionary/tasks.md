...
- [x] T068 Slice 2 — add `src/ButtonPanelTester.Services/Dictionary/WarmUp.fs` exposing `runAsync : IDictionaryProvider -> IClock -> ILogger -> CancellationToken -> Task<unit>`: calls `FetchAsync` once, measures elapsed wall-clock via `IClock`, logs the duration and the result classification (`Success` vs `Failed reason`) at `Information`, propagates `OperationCanceledException` originating from the caller's `ct` but swallows any other outcome; invoke it from `src/ButtonPanelTester.GUI/App.fs` `MainWindow.Opened` alongside `service.InitializeAsync` so the warm-up runs fire-and-forget once per launch. Tests: new `tests/ButtonPanelTester.Tests/Unit/WarmUpTests.fs` with a tiny stub `IDictionaryProvider` asserting (a) `FetchAsync` is called exactly once; (b) `runAsync` returns successfully when the provider returns `Failed(_, _)`; (c) `runAsync` propagates `OperationCanceledException` when `ct` is cancelled by the caller. Files: `src/ButtonPanelTester.Services/Dictionary/WarmUp.fs`, `src/ButtonPanelTester.Services/ButtonPanelTester.Services.fsproj`, `src/ButtonPanelTester.GUI/App.fs`, `tests/ButtonPanelTester.Tests/Unit/WarmUpTests.fs`, `tests/ButtonPanelTester.Tests/ButtonPanelTester.Tests.fsproj`.

**Checkpoint (Phase 7 complete)**: T067 + T068 ticked; `dotnet build -c Release` green; `dotnet test` green across both TFM legs; `dotnet format --verify-no-changes` green; cold-start probe re-run against the production service is an operator follow-up tracked outside this PR.
