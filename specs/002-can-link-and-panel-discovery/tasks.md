...
- [ ] T011 [P] Add `src/ButtonPanelTester.Infrastructure/Can/VENDOR-GUARD.md` — one-line README inside the new `Can/` folder pointing at [contracts/vendor-manifest.md](./contracts/vendor-manifest.md) §"Re-vendoring procedure" so a future contributor opening the F# adapter folder is immediately warned about the freeze + re-vendor discipline before touching the C# project.
...
- [ ] T026 [P] Add `tests/ButtonPanelTester.Tests/Property/Can/CanLinkStateTransitionsProperties.fs` — FsCheck property: `CanLinkStateTransitions` (starting from any reachable `CanLinkState`, applying any input event lands in another reachable state per the Lean spec in T027). Depends on T012.
- [ ] T027 [P] Add `lean/Stem/ButtonPanelTester/Phase2/CanLinkState.lean` — define the `CanLinkState` inductive (closed, mirroring T012); prove `theorem state_classification_total`: the five top-level classifications partition the state space. No `sorry`, no custom axioms.
...
- [ ] T043 [P] [US1] Add `tests/ButtonPanelTester.Tests.Windows/Integration/Can/Hardware/PcanLifecycleTests.fs` — `[<Trait("Category", "Hardware")>]` (excluded from default CI; tracked by [#112](https://github.com/luca-veronelli-stem/panel-tester/issues/112)). Cases: real PEAK adapter present + `OpenAsync 250000` succeeds within 2 s and surfaces `Connected` with non-empty `AdapterIdentification`; `CloseAsync` followed by `OpenAsync` succeeds; physical unplug surfaces `Disconnected(MidSessionUnplug, _)` within 5 s.
...
- [ ] T070 SC-007 verification with an external bus capture: connect a second CAN listener (PCAN-View on a sibling channel, or a known-good silent CAN sniffer) and record a 10-minute trace while the tool runs through the full quickstart sequence (boot → Connected → observe panel → unplug → replug → Reconnect → observe again). Confirm the captured trace shows zero frames originating from the tool's CAN ID range. Attach the trace summary to the PR.
