...
- **FR-002a**: When the state is Error, the classification MUST be surfaced **both in the chip headline and in the detail affordance** (the latter shows the technical context, the former is the at-a-glance signal). The chip headline takes the form `Recoverable · <detail>` or `Fatal · <detail>` — the "Error" prefix is omitted because the red chip already carries the state family (see "Presentation surfaces" above). The detail string MUST be a single line of the form `<cause> — <imperative suggestion>` joined by em-dash; the row renders this verbatim (first line only if the underlying detail is multi-line), the detail affordance carries the full multi-line context. The two sub-classifications:
  - **Recoverable** — a reconnect click may clear it (e.g., bus-off detected by the CAN controller, transient unexpected PEAK driver status code). The imperative suggestion is "try reconnect" (escalation guidance lives in the multi-line tooltip when needed).
  - **Fatal** — the technician must take external action (e.g., PEAK driver not installed, hardware failure, persistent unrecognised PEAK status). The imperative suggestion is the concrete external step ("install the PEAK driver", "restart the tool", "file bug with the status code").
