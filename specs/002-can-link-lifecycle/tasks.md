...
- [x] T069 End-to-end validation: walk `quickstart.md` §"Expected behaviour on a clean bench" steps 1, 2, 4, 5 on a real bench; verify SC-001, SC-002, SC-005, SC-008. (Step 3 — panel observation — is spec-003's validation.) SC-007 verified separately in T070. → **Manual — operator-attested on the bench 2026-06-03** (SC-001/002/005/008 all observed passing); no log retained. SC-001/002/005/008 are also independently covered by the automated suite; runbook in `quickstart.md` §"Expected behaviour on a clean bench".
- [x] T070 SC-007 verification with an external bus capture. Confirm zero frames originating from the tool. Attach the trace summary to the PR. → **Manual — operator-attested on the bench 2026-06-03** (independent capture showed zero frames from the tool); the capture trace was not retained, so no summary is attached. Also mechanised in Lean (`Phase2/PassiveObserver.lean`, `observe_emits_no_transmit`).
