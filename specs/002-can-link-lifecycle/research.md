...
**Rationale**: feat-001's `DictionaryStatusRow` established the precedent — pill colour = source truth (Live / Cached), pill opacity drop + headline ellipsis + spinner button = click acknowledgement. The CAN row inherits the same two-layer separation so the operator's "what is true" channel and "did my click do something" channel stay clean across both surfaces of the main window. A sub-perceptual call (e.g. Start, or a fast Reconnect) is consistent with truth-to-state — the operator's signal that the click was processed comes from the resulting FSM transition (chip colour change and/or `since` update), not from the cue itself.
