...
# Phase C focused proof: the production button-state observer adapter (Windows leg).
dotnet test tests/ButtonPanelTester.Tests.Windows/ButtonPanelTester.Tests.Windows.fsproj -c Release --filter "Category!=Hardware&FullyQualifiedName~ButtonStateReassemblyObserver"
if ($LASTEXITCODE -ne 0) { $failures += 'focused: ButtonStateReassemblyObserver' }
