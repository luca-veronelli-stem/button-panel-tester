...
# Phase B focused proof: the per-variant button-schema suite.
dotnet test tests/ButtonPanelTester.Tests/ButtonPanelTester.Tests.fsproj --framework net10.0 -c Release --filter "Category!=Hardware&FullyQualifiedName~ButtonSchema"
if ($LASTEXITCODE -ne 0) { $failures += 'focused: ButtonSchema' }
